...
def api_input_design(api_inputs: list) -> json:
    ...
    json_ip = json.dumps(dict(enumerate(input_arr.flatten(), 1)))
    print(json_ip)
    return json_ip
...
def questionnaire():
    ...
    if st.button("Submit", type='primary'):
        ...
        if all_answered:
            ...
            json_dict = {"test_data": flatten_arr}
            persona_response = requests.post(url, json=json_dict, headers={"Content-Type": "application/json"})
            persona = list(dict(persona_response.json()[0]).values())[0]
            probability = list(dict(persona_response.json()[0]).values())[1]
